...
### MSA 구조
* _도메인 별 5개의 서버(Spring boot)와 5개의 데이터베이스(Maria DB)__ : 유저, 주식, 부동산p2p, 에너지p2p, 오더 서버
* _소켓 서버(NodeJs)와 체결 및 거래 DB(Mongo DB)_ : 실시간 데이터 통신, 이벤트 핸들링 최적화
* _Kafka 서버_ : __비동기 통신__. kafka 2.8.0, Zookeeper 등
### 서버 라우팅 & 로드 밸런싱
